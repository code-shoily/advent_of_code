...
solve1(Data) ->
    lists:foldl(
        fun(C, Acc) -> Acc + length(exposed_sides(C, Data)) end,
        0,
        sets:to_list(Data)
    ).
...
is_within_confines({X, Y, Z}, {X1, X2, Y1, Y2, Z1, Z2}) when
    X >= X1 - 1, X =< X2 + 1, Y >= Y1 - 1, Y =< Y2 + 1, Z >= Z1 - 1, Z =< Z2 + 1
->
    true;
...
exposed_cubes([C | Rest], Confines, Set, Acc) ->
    ...
    exposed_cubes(
        ordsets:union(Rest, ordsets:from_list(ToFollow)),
        Confines,
        Set,
